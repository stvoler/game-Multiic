...
function Cloud() {
    ...
    Cloud.prototype.Draw = function(time, delta) {
        ...
        if(this.mesh.position.z > 200) {
            ...
            this.mesh.position.x = Math.random()*89;
            this.mesh.position.y = 2+Math.random()*2;
        }
        if(this.snow) {
...
function Lava() {
    ...
    Lava.prototype.Create = function(scene) {
        ...
        var texture = THREE.ImageUtils.loadTexture( "textures/lava4.png" );
...
function Water() {
    ...
    Water.prototype.Create = function(scene) {
        ...
        var texture = THREE.ImageUtils.loadTexture( "textures/water.png" );
        texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set( 30, 30 );
...
function Coal() {
    ...
    Coal.prototype.Create = function(scene) {
        var width = 400;
        var depth = 400;
        var geometry = new THREE.PlaneGeometry( width, depth, 64 - 1, 64- 1 );
        geometry.applyMatrix( new THREE.Matrix4().makeRotationX( - Math.PI / 2 ) );
        ...
        var texture = THREE.ImageUtils.loadTexture( "textures/coal.png" );
        texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set( 30, 30 );
        ...
        var mesh = new THREE.Mesh(geometry, material);
        mesh.position.set(50, game.currentMap.coalPosition, 50);
        //mesh.receiveShadow = true;
        this.mesh = mesh;
        scene.add(this.mesh);
    };

    Coal.prototype.Draw = function(time, delta, i) {
        for ( var i = 0, l = this.mesh.geometry.vertices.length; i < l; i ++ ) {
         //   this.mesh.geometry.vertices[ i ].y = 0.1 * Math.sin( i / 5 + ( time + i ) / 7 );    
            this.mesh.geometry.vertices[ i ].y = 0.2 * Math.sin( i / 5 + ( time + i ) / 4 );
        }
