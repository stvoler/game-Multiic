...
function Player() {
    ...
    Player.prototype.CreateHealth = function() {
        ...
        for(var i=0; i < this.bars; i++) {
            var geo = new THREE.BoxGeometry(0.08, 0.1, 0.03);
            var color;
            switch(i) {
                case 0:
                    color = 0xb72424;
            ...
            b.position.set((i*0.1+0.01)-(this.bars/2*0.1), 0, 1.3);
    ...
    Player.prototype.Create = function(model, pos) {
        ...
        game.camera.position.set(0, 7, 3);
        //this.mesh.rotation.set(Math.PI/2, Math.PI, 0);
        game.camera.rotation.set(-Math.PI/2.3, 0, Math.PI);
 ...
 //       this.mesh.rotation.set(Math.PI/2, Math.PI, 0);
    ...
    Player.prototype.Die = function(fall) {
        ...
        if(fall) {
            $('#statusCenter').text("You slipped and died!");
        ...
        $('#statusCenter').text("Выкуси!");
        $('#statusCenter').fadeIn(1000);
        this.chunk.Explode(this.mesh.position);
        this.dead = true;
        this.remove = 1;
        /*
        if(game.currentMap.id == 5) {
            var secretMap = 5;
            game.ReInit(secretMap);
        } else {
            setTimeout(function() {
